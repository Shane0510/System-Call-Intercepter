...
void my_exit_group(int status) {
	// call exit group after removed pid from all lists
...
asmlinkage long interceptor(struct pt_regs reg) {
	// log message and call the original syscall
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	if (cmd == REQUEST_SYSCALL_INTERCEPT) {
		//check if caller is the root
		if (current_uid() != 0) {
			return -EPERM;
		}
		spin_lock(&my_table_lock);
		//check if system call is already intercepted.
	...
	//de-intercept a system call
		...
		spin_lock(&my_table_lock);
		//check if system call has not been intercepted yet.
	...
	//add a pid to monitor list of a system call 
	} else if (cmd == REQUEST_START_MONITORING) {
		//check if the pid is valid
		if((pid < 0) || ((pid_task(find_vpid(pid), PIDTYPE_PID) == NULL) && (pid != 0))){
			return -EINVAL;
		}
		//check if the pid requested is owned by the calling process while calling process not root
		if ((current_uid() != 0) && (pid == 0 || (check_pids_same_owner(pid, current->pid) != 0))) {
			return -EPERM;
		}
		spin_lock(&my_table_lock);
		//check if system call monitored all pid
		if (table[syscall].monitored != 2) {
			//check if pid already monitor by system call
			if (check_pid_monitored(syscall, pid) == 1) {
				spin_unlock(&my_table_lock);
				return -EBUSY;
			}
			//check if all pid need to be add 
			if (pid == 0) {
				destroy_list(syscall);
				table[syscall].monitored = 2;
			//check if a pid can be added to monitored list
			...
			//check if any pid deleted from systen call monitored all pid
			if (table[syscall].listcount != 0) {
				//check if all pid need to be add to monitored list again
				if (pid == 0) {
					destroy_list(syscall);
					table[syscall].monitored = 2;
				} else {
					//check if pid requested is deleted from systen call monitored all pid before
					if (check_pid_monitored(syscall, pid) == 1) {
						del_pid_sysc(pid, syscall);
					}
				}
			//try to monitor a pid that is already being monitored.
			} else {
				spin_unlock(&my_table_lock);
				return -EBUSY;
			}
	...
	//stop monitor a pid for a system call
		...
		spin_lock(&my_table_lock);
		//check if system call hasn't been intercepted
		if (table[syscall].intercepted == 0) {
			spin_unlock(&my_table_lock);
			return -EINVAL;
		}
		//check if pid hasn't been monitored by system call
...
static int init_function(void) {
	...
	// store sys call to orig functions
	orig_custom_syscall = sys_call_table[MY_CUSTOM_SYSCALL];
	orig_exit_group = sys_call_table[__NR_exit_group];
	// hijack MY_CUSTOM_SYSCALL and exit_group call
	set_addr_rw((unsigned long)sys_call_table);
...
static void exit_function(void){
	...
	// restore syscall table to original
	set_addr_rw((unsigned long)sys_call_table);
